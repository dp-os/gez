...
패키지 관리자를 사용하여 `@gez/rspack-vue` 개발 의존성을 설치합니다:
...
빌드 대상 환경 타입으로, 애플리케이션의 빌드 대상 환경을 정의하며, 빌드 과정에서 특정 최적화 및 기능을 구성하는 데 사용됩니다:
...
- `styleLoader`: CSS를 DOM에 주입하기 위한 loader
- `cssLoader`: CSS 파일을 해석하고 CSS 모듈화를 처리하기 위한 loader
- `lessLoader`: Less 파일을 CSS로 컴파일하기 위한 loader
- `styleResourcesLoader`: 전역 스타일 리소스(예: 변수, mixins)를 자동으로 가져오기 위한 loader
- `workerRspackLoader`: Web Worker 파일을 처리하기 위한 loader
...
`@rspack/core` 패키지의 모든 내용을 다시 내보내어, Rspack의 핵심 기능을 완벽하게 제공합니다.
```