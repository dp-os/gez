...
description: Ferramenta de construção especializada em Vue do framework Gez, oferece suporte completo para construção de aplicativos Vue 2/3, incluindo desenvolvimento de componentes, renderização SSR e otimização de desempenho.
head:
  - - meta
    - property: keywords
...
Tipo de ambiente de construção, define o ambiente de destino para o qual o aplicativo será construído, usado para configurar otimizações e funcionalidades específicas no processo de construção:
...
**Observações:**
- Esses loaders já estão embutidos no Rspack, não é necessário instalá-los separadamente
