...
Installez `@gez/rspack-vue` en tant que dépendance de développement à l'aide d'un gestionnaire de paquets :
...
- `styleResourcesLoader` : Loader utilisé pour importer automatiquement des ressources de style globales (comme des variables, des mixins)
- `workerRspackLoader` : Loader utilisé pour traiter les fichiers Web Worker

Utiliser ces constantes permet de référencer les loaders intégrés dans la configuration, évitant ainsi de saisir manuellement des chaînes de caractères :
