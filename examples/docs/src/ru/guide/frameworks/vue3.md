...
После создания файла `package.json` установите зависимости проекта с помощью одной из следующих команд:
```bash
pnpm install
# или
yarn install
...
Это установит все необходимые пакеты, включая Vue3, TypeScript и зависимости для SSR.
...
export default {
     ...
     * @description Создает и настраивает экземпляр приложения Rspack для разработки, поддерживающий HMR и live preview
...
1. `devApp`: отвечает за создание и конфигурацию экземпляра приложения Rspack для разработки, поддерживающего горячую перезагрузку и live preview. Здесь используется `createRspackVue3App` для создания экземпляра Rspack, оптимизированного для Vue3.
2. `server`: отвечает за создание и конфигурацию HTTP-сервера, интегрирующего middleware Gez для обработки SSR-запросов.
 ...
 * @description Отвечает за процесс SSR, генерацию HTML и инъекцию ресурсов
 */
...
export default async (rc: RenderContext) => {
    ...
    // Фиксация собранных зависимостей для обеспечения загрузки всех необходимых ресурсов
    await rc.commit();
