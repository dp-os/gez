...
2. **Интеллектуальный сбор зависимостей**
   ...
   - Избегает ненужной загрузки ресурсов
...
3. **Точное внедрение ресурсов**
   ...
   - Обеспечивает надежность гидратации на стороне клиента
...
- Собирает только те ресурсы, такие как CSS и JavaScript, которые действительно используются на текущей странице
...
- `preload()`: Предварительно загружает ресурсы CSS и JS с поддержкой настройки приоритетов
- `css()`: Внедряет стили для первой страницы с поддержкой извлечения критического CSS
- `importmap()`: Внедряет карту импорта модулей с поддержкой динамического разрешения путей
- `moduleEntry()`: Внедряет клиентский входной модуль с поддержкой конфигурации нескольких входов
- `modulePreload()`: Предварительно загружает зависимости модулей с поддержкой стратегии загрузки по требованию
...
1. Часть head:
   - `preload()`: Предварительно загружает ресурсы CSS и JS, чтобы браузер мог начать их загрузку как можно раньше
   - `css()`: Внедряет стили для первой страницы, чтобы обеспечить готовность стилей при рендеринге контента

2. Часть body:
   - `importmap()`: Внедряет карту импорта модулей, определяя правила разрешения путей для ESM-модулей
   - `moduleEntry()`: Внедряет клиентский входной модуль, который должен выполняться после importmap
   - `modulePreload()`: Предварительно загружает зависимости модулей, что должно выполняться после importmap
...
RenderContext предоставляет гибкий механизм динамической настройки базового пути, который позволяет устанавливать базовый путь для статических ресурсов во время выполнения:
...
2. **Микрофронтенд-приложения**
   - Поддерживает гибкое развертывание подприложений на разных путях
   - Упрощает интеграцию в различные главные приложения
...
Режим можно выбрать через конфигурацию:
...
### Конфигурация входной функции

RenderContext поддерживает настройку `entryName` для указания входной функции для серверного рендеринга:
...
1. **Множественные шаблоны рендеринга**
   ```ts title="src/entry.server.ts"
...
2. **A/B-тестирование**
   - Поддерживает использование различных логик рендеринга для одной страницы
   - Упрощает проведение экспериментов с пользовательским опытом
   - Позволяет гибко переключаться между различными стратегиями рендеринга

3. **Специальные требования к рендерингу**
   - Поддерживает использование пользовательских процессов рендеринга для определенных страниц
   - Адаптируется к различным сценариям оптимизации производительности
...
1. **Получение экземпляра RenderContext**
   ...
   - Избегайте ручного создания экземпляра
...
4. **Оптимизация производительности**
   ...
   - Избегайте ненужной загрузки ресурсов
