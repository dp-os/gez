...
- **内容哈希与importmap**：构建产物具备内容哈希，利用 [importmap](https://caniuse.com/?search=import%20map) 技术将 `import vue from 'vue'` 转换为 `ssr-npm/npm/vue.[contenthash].final.mjs`，确保静态文件的强缓存。对于不支持importmap的浏览器，采用 [es-module-shims](https://github.com/guybedford/es-module-shims) 进行降级处理。
