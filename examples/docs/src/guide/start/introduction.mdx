...
## 为什么选 Gez
目前社区类微服务的解决方案基本可以分为 [iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe)、[micro-app
](https://github.com/micro-zoe/micro-app)、[Module Federation](https://module-federation.io/) 三种代表。其中 iframe 和 micro-app 这种模式只适合对已有的老项目进行缝合，是以降低程序运行效率所做的一种妥协，而 Module Federation 的接入成本较高，里面又是一个黑盒子，一旦出了问题，都十分难以排查。
...
- 在 Node 上运行 TypeScript 并不是一件复杂的事情，但是在 Node22.6 版本支持 `--experimental-strip-types`，可以原生支持运行 TS 了。
...
## 定位
Gez 的定位并不是成为 [Next.js](https://nextjs.org) 或 [Nuxt.js](https://nuxt.com/) 那样大而全的框架，而是成为一个具有 Typescript、ESM、SSR、[模块链接](/guide/essentials/module-link.mdx)等特性的基础设施，你可以在这个基础上来构建属于你自己的 `Next.js`。如果你需要定制化实现，它会很适合你。
