...
要在生产环境中使用客户端渲染，需要在构建阶段生成静态的 HTML 文件。Gez 提供了 `postCompileProdHook` 钩子函数来实现这一功能。
...
::: tip 提示
`postCompileProdHook` 钩子会在生产环境构建完成后执行。你可以利用这个钩子不仅生成客户端渲染所需的 HTML 文件，还可以实现完整的静态站点生成（SSG）功能。
:::
