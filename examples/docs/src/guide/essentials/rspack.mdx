...
::: tip
Gez 提供了 `@gez/rspack` 的 `createRspackHtmlApp` 函数，支持构建 [Vue](../framework/vue.mdx)、[React](../framework/react.mdx)、[Preact](../framework/preact.mdx)、[Solid](../framework/solid.mdx)、[Svelte](../framework/solid.mdx) 等主流前端框架的应用。
:::

## 基础配置模块 (@gez/rspack)

`@gez/rspack` 模块提供了 Rspack 的基础配置能力。
