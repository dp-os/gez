...
description: Gez フレームワークの Rspack ビルドツールは、高性能なアプリケーションビルド機能を提供し、標準アプリケーションと HTML アプリケーションの開発とビルドをサポートします。複数のリソースプロセッサと最適化設定が組み込まれています。
head:
  - - meta
    - property: keywords
...
パッケージマネージャを使用して `@gez/rspack` 開発依存関係をインストールします：
...
Rspack アプリケーション設定コンテキストのインターフェースです。設定フック関数内でアクセス可能なコンテキスト情報を提供します：
...
Rspack アプリケーション設定オプションのインターフェースです：
...
Rspack に組み込まれた loader 識別子のマッピングオブジェクトです。一般的に使用される loader 名の定数を提供します：

- `builtinSwcLoader`: Rspack に組み込まれた SWC loader。TypeScript/JavaScript ファイルを処理します
- `lightningcssLoader`: Rspack に組み込まれた lightningcss loader。CSS ファイルの高性能コンパイラです
...
- `styleResourcesLoader`: グローバルスタイルリソース（変数、mixins など）を自動インポートするための loader
