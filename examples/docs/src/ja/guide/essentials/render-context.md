...
async server(gez) {
    const server = http.createServer((req, res) => {
        ...
        gez.middleware(req, res, async () => {
            // gez.render() で RenderContext インスタンスを取得
...
- `commit()` メソッドで収集されたすべてのリソースを一括処理
- 循環依存と重複依存の問題を自動的に処理

#### パフォーマンス最適化
- 未使用のモジュールのロードを回避し、初回ロード時間を大幅に削減
...
RenderContext は、異なるタイプのリソースを注入するための複数のメソッドを提供しており、各メソッドはリソースロードパフォーマンスを最適化するために設計されています：
...
- `moduleEntry()`：クライアントサイドのエントリモジュールを注入し、マルチエントリ設定をサポート
- `modulePreload()`：モジュール依存関係をプリロードし、オンデマンドロード戦略をサポート

### リソース注入順序

RenderContext はリソース注入順序を厳密に制御しており、この順序設計はブラウザの動作原理とパフォーマンス最適化を考慮しています：

1. head 部分：
   - `preload()`：CSS と JS リソースをプリロードし、ブラウザがこれらのリソースを早期に発見してロードを開始できるようにします
   - `css()`：初回表示用のスタイルシートを注入し、ページのスタイルがコンテンツレンダリング時に準備されるようにします

2. body 部分：
   - `importmap()`：モジュールインポートマッピングを注入し、ESM モジュールのパス解決ルールを定義します
   - `moduleEntry()`：クライアントサイドのエントリモジュールを注入し、importmap の後に実行する必要があります
...
2. **JS モード**
   ...
   - 大規模アプリケーションに適し、ブラウザのキャッシュメカニズムを活用
   - マッピング内容の動的更新をサポート
...
RenderContext は `entryName` 設定を通じてサーバーサイドレンダリングのエントリ関数を指定することをサポートします：

```ts title="src/entry.node.ts"
const rc = await gez.render({
    entryName: 'mobile',  // モバイルエントリ関数を使用
    params: {
        url: req.url
    }
});
...
4. **パフォーマンス最適化**
   ...
   - 不要なリソースロードを避ける
   - ブラウザのキャッシュメカニズムを活用してロードパフォーマンスを最適化