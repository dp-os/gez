...
クライアントサイドレンダリング（Client-Side Rendering、CSR）は、ブラウザ側でページのレンダリングを実行する技術的なアプローチです。Gez では、Node.js サーバーインスタンスをデプロイできない場合、ビルド段階で静的な `index.html` ファイルを生成し、純粋なクライアントサイドレンダリングを実現することができます。

## 使用シナリオ

以下のシナリオでは、クライアントサイドレンダリングの使用が推奨されます：

- **静的ホスティング環境**：GitHub Pages、CDN など、サーバーサイドレンダリングをサポートしていないホスティングサービス
- **シンプルなアプリケーション**：初期読み込み速度や SEO の要件が高くない小規模なアプリケーション
- **開発環境**：開発段階でアプリケーションを迅速にプレビューおよびデバッグする場合
...
本番環境でクライアントサイドレンダリングを使用するには、ビルド段階で静的な HTML ファイルを生成する必要があります。Gez は `postBuild` フック関数を提供しており、この機能を実現します：
