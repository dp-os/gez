...
Gez 是 Genesis 迭代的第三个大版本，[v1.0](https://github.com/dp-os/gez/tree/v1) 是通过 HTTP 请求来实现的远程组件，[v2.0](https://github.com/dp-os/gez/tree/v2) 是通过 Module Federation v1.0
 实现的远程组件。随着主流浏览器都已经支持 ESM，这使得设计一款基于 ESM 的[模块链接](./essentials/module-link.mdx)系统变成了可能。随着 Rspack v1.0 的发布，提供了对 ESM 更加友好的支持，这使得我们可以将可能变成了现实。于是，我们将 [v3.0](https://github.com/dp-os/gez) 版本命名为 `Gez`。
...
Gez 完全是基于 ESM 模块系统进行设计，默认支持 SSR，每个服务都可以对外导出模块，也可以使用外部模块，整个过程简单透明，能够精准的控制依赖管理。在客户端可以使用 `importmap` 将不同服务的导出映射成具有内容哈希缓存的文件。
